...
export function printUsage(): void {
    console.log(
        [
            ...
            '',
            'Supported workflow (static review and spec-mapping policy):',
            '  review --path <app-root> --since <ref> [--generate] [--deep] [--json] [--ci-comment-path <path>]',
            '  gate --path <app-root> --threshold <n> [--since <ref>] [--json]',
            '  Static review needs no provider. --generate and --deep are experimental provider-backed modes.',
            '  A passing gate establishes spec presence, not measured coverage or release safety.',
            '',
            'Experimental advisory pilot (read-only, no model or test execution):',
            '  plan --advisory --json --config <config.json> --path <clean-checkout> --since <ref> --suite <id>',
            '',
            'Experimental analysis:',
            '  impact --path <app-root> [options]',
            '  plan --path <app-root> [options]',
            '  suggest --path <app-root> [options]',
            '  predict --path <app-root> --since <ref> [--deep] [--json]',
            '  predict-feedback --outcome <defect|clean> [--ref <sha>]',
            '',
            'Experimental generation and maintenance:',
            ...
            '  --json               Output structured JSON for review/gate and supported report commands',
            '  --threshold <n>      Gate spec-mapping percentage (default: 80)',
            '  --predict-threshold <0-1>  Fail when the prediction score exceeds this value',
            ...
            '  --scenarios-output <path>          Export review recommendations for generate --scenarios (no provider required)',
            ...
            '  --dry-run            Generate quarantined proposals without test execution; preview git handoff',
